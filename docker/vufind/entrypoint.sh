...
VUFIND_BASEPATH="${VUFIND_BASEPATH:-/}"
...
VUFIND_DB_HOST="${VUFIND_DB_HOST:-vufind-db}"
...
VUFIND_SOLR_URL="${VUFIND_SOLR_URL:-}"

export VUFIND_ENV
export VUFIND_LOCAL_MODULES

if [ -z "${VUFIND_SOLR_URL}" ]; then
  echo "Error: VUFIND_SOLR_URL is required. Set SOLR_EXTERNAL_URL in .env." >&2
  exit 1
fi
...
apache_setenv_escape() {
  printf '%s' "$1" | sed 's/\\/\\\\/g; s/"/\\"/g'
}

configure_apache_env() {
  apache_env_file="/etc/apache2/vufind-runtime-env.conf"
  {
    printf 'SetEnv VUFIND_LOCAL_DIR "%s"\n' "$(apache_setenv_escape "${VUFIND_LOCAL_DIR}")"
    if [ -n "${VUFIND_LOCAL_MODULES}" ]; then
      printf 'SetEnv VUFIND_LOCAL_MODULES "%s"\n' "$(apache_setenv_escape "${VUFIND_LOCAL_MODULES}")"
    fi
  } > "${apache_env_file}"
}
